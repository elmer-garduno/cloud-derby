...
def inference(image_file_name, labels, interpreter, threshold):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-file', help='tflite model file name')
    parser.add_argument('--labels', help='model labels')
    parser.add_argument('--threshold', help='model threshold')
    ...
    myCar = RobotDerbyCar()
    myCar.SetBallModeLED(myCar.PURPLE)

    voltage = myCar.ReadBatteryVoltage()
    print("main(): voltage: {}".format(voltage))
    ...
    try:
        ...
        # Number of attempts at finding the target ball, reset when found
        ball_turns = 0
		...
		# Force send_next_message to always take pictures.
                ...
                if (len(action_queue) > 0):
                    ...
                    if(command_timestamp>=startup_time):
                        ...
                        if (action_type == "driveForwardMm"):
                            ...
                            if (ball_color=="RedBall"):
                                myCar.SetBallModeLED(myCar.RED)
                            elif (ball_color=="YellowBall"):
                                myCar.SetBallModeLED(myCar.YELLOW)
                            elif (ball_color=="GreenBall"):
                                myCar.SetBallModeLED(myCar.GREEN)
                            elif (ball_color=="BlueBall"):
                                myCar.SetBallModeLED(myCar.BLUE)
                            else:
                                # No color detected set to purple
                                myCar.SetBallModeLED(myCar.PURPLE)
                ...
                ######### Once commands are processed collect picture, distance, voltage
                    ...
                    print("main(): distance Sensor (mm): {}".format(distance))
                    # Sleep briefly before taking a photo to prevent blurry images
                    # myCar.SetCarStatusLED(myCar.YELLOW)
                    time.sleep(0.1)
                    local_path = takephoto(project_var,bucket_var, camera_position)
                    objects = inference(local_path, labels, interpreter, float(args.threshold))
                    target_object = "YellowBall"
                    target_found, command_queue = navigation.navigate_to_ball(target_object, objects, labels)
                    action_queue.extend(command_queue)
                    # If no objects detected enter lookup mode
                    if target_found:
                      ball_turns = 0
                    else:
                      action_queue.extend(navigation.ball_search_strategy(ball_turns))
                      ball_turns += 1
                    print(">>>>>> {}".format(action_queue))
                    # To debug
                    # action_queue.clear()
        ...
        print(
            'Exception(): listening for messages on threw an Exception: {}.'.format(e))
        raise
